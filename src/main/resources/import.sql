insert into cozinha (id,nome) values (1,'Brasileira');
insert into cozinha (id,nome) values (2,'Japonesa');




insert into restaurante (nome, taxaFrete,cozinha_id) values ('Nacib Beach', 15.25, 1);
insert into restaurante (nome, taxaFrete, cozinha_id) values ('Boteco do Sushi', 9.25,2 );
insert into restaurante (nome, taxaFrete, cozinha_id) values ('Los Pampas', 12.35,1)
insert into estado (nome ) values('Bahia')
insert into estado (nome ) values ('Goias')
